...
def get_strength(original_hand)
    ...
    greater = nil
    # get greater value from counting that is not J
    counting.each do |key, value|
        if key == "J" then next end
        
        if greater == nil or value > counting[greater] then
            greater = key
        end
    end
    ...
    unless jokers == nil or greater == nil
        counting.delete("J")
        counting[greater] += jokers
    end    
